...
// CustomModelerTodoDone: Define which elements may be auto-placed.
export function getNewShapePosition(source, element) {
  if (
    element.type === `${MODELER_PREFIX}:State`
  ) {
...
export function getFlowNodePosition(source, element) {
  ...
  // CustomModelerTodoDone: Define which connection type is used for auto-placement.
  const horizontalDistance = getConnectedDistance(source, {
    filter: function filter(connection) {
      return connection.type === `${MODELER_PREFIX}:Transition`;
    },
