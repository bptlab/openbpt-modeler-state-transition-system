...
CustomModeler.prototype.addModelAndDiagram = function (name) {
  const model = this.get("customElementFactory").create("olc:Model", {
    name: name || "<TBD>",
  });
  this._definitions.get("models").push(model);
  const diagram = this.get("customElementFactory").create("olcDi:Diagram");
  const plane = this.get("customElementFactory").createDiPlane(model);
  diagram.plane = plane;
...
CustomModeler.prototype.deleteModelAndDiagram = function (model, diagram) {
  const currentIndex = findIndex(this._definitions.diagrams, diagram);
  const indexAfterRemoval = Math.min(
    currentIndex,
